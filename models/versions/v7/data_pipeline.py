...
class DataPipeline:
    ...
    def load_daily(self) -> pd.DataFrame:
        ...
        # Handle typical schema: timestamp (epoch seconds), open, high, low, close, volume
        ...
        # Rename price/volume columns to lowercase canonical names
        rename_map = {}
        ...
        # 'date' is already naive if created from timestamp; otherwise make sure it's naive
        if isinstance(date, pd.Series):
            if hasattr(date.dt, 'tz'):  # pandas will provide .dt.tz for tz-aware
                try:
    ...
    def load_intraday(self) -> pd.DataFrame | None:
        ...
        # Expect columns with timestamp and OHLCV (timestamp epoch seconds)
        ...
        idf['ts'] = ts
        # Align to local date for India markets
    ...
    def compute_intraday_aggregates(self, idf: pd.DataFrame) -> pd.DataFrame:
        if idf is None:
            return pd.DataFrame()
        # Previous-day intraday stats by date
        ...
        # Shift by 1 day so these describe previous day's intraday behavior for next-day prediction
    ...
    def engineer_features(self, ddf: pd.DataFrame, id_agg: pd.DataFrame | None) -> pd.DataFrame:
        ...
        # RSI, ATR if talib available; else simple approximations
        ...
        # Additional short-term momentum RSIs and diffs
        for p in [3, 8, 21]:
            if talib is not None:
        ...
        # Merge intraday aggregates (shifted to next day)
    ...
    def postprocess_ohlc_from_rel(base_close: float, open_rel: float, dh: float, dl: float, dc: float, atr_pct: float | None, clamp_mult: float = 3.5) -> Tuple[float, float, float, float]:
        """Reconstruct absolute OHLC from relative targets and apply ATR-based clamping and bounds.

        base_close: today's close
        open_rel: (open_next - close_today)/close_today
        dh, dl, dc: relative deltas vs open_next
        atr_pct: ATR(14)/Close_t if available
        clamp_mult: cap |dh|, |dl| to clamp_mult * atr_pct
        """
        # Clamp relative range if atr_pct available
        if atr_pct is not None and not (np.isnan(atr_pct) or np.isinf(atr_pct)):
            cap = max(atr_pct * clamp_mult, 0.01)  # minimum 1% to avoid degenerate zero ranges
        ...
        # Enforce sanity: L ≤ min(O,C,H), H ≥ max(O,C,L)
        hi = max(o, h, l, c)
        lo = min(o, h, l, c)
        # If low > min, snap low down, and if high < max, snap up
        l = min(l, o, c, h)
        h = max(h, o, c, l)
        # If still inverted due to extreme dc, squeeze into [lo, hi]
        if l > h:
            mid = (o + c) / 2.0
    ...
    def make_targets(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Relative open vs today's close (stabilizes target scale)
        # open_rel = (open_{t+1} - close_t) / close_t
    ...
    def sequence_dataset(df: pd.DataFrame, feature_cols: List[str], lookback: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[pd.Timestamp], np.ndarray]:
        ...
        for i in range(lookback, len(df) - 1):  # -1 for next-day availability already considered
            # row i corresponds to predicting day i (which is df index i with next-shift already done)
...
def get_default_feature_columns(df: pd.DataFrame) -> List[str]:
    ...
    # Optional direction-focused features
    ...
    # Intraday aggregates if present
    ...
    for c in ['dow_1', 'dow_2', 'dow_3', 'dow_4', 'dow_5', 'month_1', 'month_3', 'month_6', 'month_9', 'month_12']:
        if c in df.columns:
            base_cols.append(c)
    # Drop any missing columns safely
